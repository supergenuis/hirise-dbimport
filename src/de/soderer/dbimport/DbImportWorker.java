...
import de.soderer.dbimport.utilities.db.DbDefinition;
...
import de.soderer.dbimport.utilities.db.SimpleDataType;
...
public class DbImportWorker extends WorkerSimple<Boolean> {
	...
	public String getResultStatistics() {
		...
		if (invalidItems.size() > 0) {
			...
			for (int i = 0; i < Math.min(10, invalidItems.size()); i++) {
				if(invalidItemsReasons!=null && invalidItemsReasons.size()>=i+1) {
					errorList.add(invalidItems.get(i) + ": " + invalidItemsReasons.get(i));
				}else{
	...
	public static String convertMappingToString(final Map<String, Tuple<String, String>> mapping) {
		...
		if (mapping != null) {
			List<Entry<String,Tuple<String,String>>> sortedListOfMappingEntries = mapping.entrySet().stream().sorted(Map.Entry.<String, Tuple<String, String>>comparingByKey()).collect(Collectors.toList());
